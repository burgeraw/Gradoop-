 ...
 */
public abstract class NewSummaryAggregation<K, EV, S extends Serializable, T> implements Serializable {
    ...
    protected NewSummaryAggregation(EdgesFold<K, EV, S> updateFun, ReduceFunction<S> combineFun,
                                 MapFunction<S, T> transform, S initialValue, boolean transientState) {
    ...
    private final static class Merger<S extends Serializable> extends WrappingFunction<ReduceFunction<S>>
            implements FlatMapFunction<S, S>, ListCheckpointed<S> {
         ...
         */
        @Override
        public List<S> snapshotState(long l, long l1) {
            return Collections.singletonList(summary);
        }

        @Override
        public void restoreState(List<S> list) {
            summary = list.get(0);
        }
    }
}